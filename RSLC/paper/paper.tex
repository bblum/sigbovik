...
In the normal $\lambda$-calculus, in the {\sc Subst-Capture} rule, substitution stops when a new binding's name is the same as the one being substituted for, but in RSLC we switch to a second substitution mode, {\sc Capture}, in which substituted expressions are appended to a variable's $\rep{e}$ list (in the {\sc Capture-Var} rule).
Similarly, when substituting for a variable before switching to {\sc Capture} mode (in the {\sc Subst-Var} rule), we ... hmm, why is two-moded substitution needed with DRS anyway?
...
\begin{figure}[t]
	...
	\caption{RSLC substitution rules. The ``$e_1 \in ...$'' premise in {\sc Subst-Var-X} represents nondeterministic choice. (The complete set of rules is shown in Figure~\ref{fig:subst-complete}.)}
	\label{fig:subst}
\end{figure}
