...
\begin{figure}[t]
	...
	\end{tabular}
	%\begin{tabular}{c}
	%	\includegraphics[width=0.16\textwidth]{false-positive-toromi-hearts.png}
	%	\\
	%	(a) Toromi Hearts 2 \\
	%	(18, Cirque du Zonda)
	%\end{tabular}
...
\newcommand\percentmatch{\ensuremath{\%_{\mathfrak{match}}}}
\begin{algorithm}[h!]
	\SetKwInOut{Input}{Input}
	\SetKwInOut{Output}{Invariant}
	\Input{\Stream, a step sequence}
	...
	\Input{$\percentmatch$, heuristic percentage, initially 0}
	\For{$i \in \mathsf{length}(\Stream) \wedge \neg {\sf defined}(i_{\Doublestep})$}{
		%$\mathcal{S}' \leftarrow \{ s_j | s_j \in \Stream \wedge j \ge i \}$ \\ %i \le j < i + \nflip \})$ \\
		$\Stream' \leftarrow \{ s_k | s_k \in \mathsf{LRs}(\{ s_j | s_j \in \Stream \wedge j \ge i \}) \wedge k \le \nflip \}$ \\
		%$\mathcal{X} \leftarrow \mathsf{CrossoverSteps}(\{ s_j | s_j \in \Stream \wedge i \le j < i + \nflip \})$ \\
		%$\mathcal{M} \leftarrow \mathsf{MatchingSteps} (\{ s_j | s_j \in \Stream \wedge i \le j < i + \nflip \})$
		\If{$|\Stream'| = \nflip \wedge |\mathsf{Matching}(\Stream')| \le \percentmatch \times (|\mathsf{Crossovers}(\Stream')| + |\mathsf{Matching}(\Stream')|)$}{
...
The heuristic evaluated Figure~\ref{fig:force-doublestep}'s charts as having 0 crossovers and 1 and 21 doublesteps, respectively.
