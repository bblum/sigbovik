...
and owing to an inferior conference's deadline coming up in 10 days \citepalias{ode}, we don't have time for re-running experiments \citepalias{ode}.
...
Although they measure the same $\mathcal{T}$ value of 1337, % TODO
(b) is considerably more exhausting to step, as (a) allows the player to bracket the entire run.
Future work could handle this by counting a 1-turn step as the full value only if the player subsequently continues turning farther in the same direction (which would force the player's feet off the brackets), and otherwise assign $\mathcal{T}=1/2$ for such a step.
