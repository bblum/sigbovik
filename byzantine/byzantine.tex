...
Let us first think about the situation pictured in Figure~\ref{fig:1} in which the first head is a good guy, and sends an "attack" order, but the second friend is a bad guy and tells the first friend that he heard a ``run away'' order. For FA2 to happen, the first friend must listen to the order to attack.

Now consider another situation, shown in Figure~\ref{fig:2}, in which the first head is actually a bad guy, and sends an ``attack'' order to the first friend and a ``run away'' order to the second friend. The first friend does not know who the bad guy is, and she can't tell what the first head actually said to the second head. So, the situations in these two pictures appear exactly the same to the first friend. If the bad guy lies all the time, then there is no way for the first friend to know which situation is happening, so she must listen to the ``attack'' order in both of them. So, any time the first friend hears an ``attack'' order from the first head, she must listen to it.
...
\begin{figure}[h]
	TODO
	\caption{The first head is a bad guy.}
	\label{fig:2}
\end{figure}

However, in the same way we could show that if the second friend hears a ``run away'' order from the first head then he must listen to it even if the first friend tells him that the head said "attack". Because of this, in Figure 2, the second friend must listen to the ``run away'' order while the first friend listens to the ``attack'' order, which means situation FA1 doesn't happen. So, there is no way for three people to agree if one of them is a bad guy.
...
\begin{enumerate}
	...
	\end{enumerate}
	\item Ask the other not-first head what they did in step (1).
	\begin{enumerate}
...
It follows from FA2' that if the first head is a good guy, then her good guy friends will hear the right order in step (1), so situation FA2 happens. Also, FA1 follows from FA2, so we only need to make sure of FA1 when we suppose the first head is a bad guy. Since there is at most one bad guy, this means that both of her friends are good guys.
...
The answers that we give in this part of the paper and in the following one only work if each head is able to speak to each other head without needing anyone in between to help. In the part of this paper number five, we talk about answers which do not need this to happen.

A bad guy first-head may decide not to send any order. Since the other heads must decide to follow some order, they need something to do if no-one tells them to do anything. We'll say RUN AWAY will be this order.

We will show how to run a Spoken Words plan, which we call SW($m$), for all numbers $m$, by which a first-head sends an order to the rest of the heads. We'll talk about this plan by a way of supposing that, as long as the plan works for some number of heads, it can also work for one more than that many heads. So if we can show that, then we know it works for all possible numbers of heads.

We show that SW($m$) is an answer to the \prob for more than three times $m$ many heads, as long as there are $m$ or fewer bad guys.

The Spoken Words plan where $m$ is nothing at all (which means there are no bad guys): \begin{enumerate}
	\item The first head says what he decided to do to every other head.
	\item Each other head does the thing they heard the first head say, or does RUN AWAY if they didn't hear anything.
\end{enumerate}

The Spoken Words plan where $m$ is one more than some other number $j$:
\begin{enumerate}
	\item The first head says what he decided to do to every other head.
	\item For each number $i$, let $\fact_i$ be the fact that head number $i$ heard from the first head, or else be RUN AWAY if he hears nothing. Head number $i$ acts as the first-head in the Spoken Word plan for one fewer than $m$, to say $\fact_i$ to the other heads.
...
To understand how this plan works, we consider the case where $m$ is one and there are four total heads. Figure~\ref{fig:3} shows what the second head hears when the first head says some fact $f$, and the third head is a bad guy.
...
Next, we see what happens if the first head is a bad guy. Figure~\ref{fig:4} shows what the other heads hear if the first head says whatever he wants, which we'll say are $x$, $y$, and $z$, because they can all be different.
...
\begin{figure}[h]
	TODO
	\caption{Plan SW(1), where the third head is a bad guy.}
	\label{fig:3}
\end{figure}
\begin{figure}[h]
	TODO
	\caption{Plan SW(1), where the first head is a bad guy.}
	\label{fig:4}
\end{figure}
