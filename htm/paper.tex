...
the access sequence is committed to main memory atomically (with respect to visibility by other CPUs).
...
% だからこそ
obscure interleavings between failure paths are difficult to expose during stress testing.
...
TSX was implemented on consumer hardware for the first time by Intel's Haswell architecture \cite{htm-haswell},
...
%\begin{itemize}
	...
	%\item
		...
		Moreover, we conclude that even Figure~\ref{fig:htm-fixed}'s protocol's very proposer % こそさえ
...
\end{figure}
	...
	%\item
		...
		Quicksand's ability to find bugs in fewer distinct interleavings %overall smaller state spaces
...
We noticed significant slowdowns when varying several aspects of our experimental environment.
For example, multiple Landslides running at once
%on our test machine
...
Because Quicksand repeats more work across jobs,
rather than comparing it to a baseline,
we show in the right column how many total interleavings each approach executed.
(The maximal state space alone comprises 384 in all cases.)
...
to provide reader some assurance of similar consistency in the larger tests
...
For all tests outside of Table~\ref{tab:variants},
we fixed our environment by measuring all performance numbers
...
which one would expect to show 2\% variance if extrapolating from these results. %this experiment.
...
\begin{table}[t]
	...
	\begin{tabular}{l|c|c|c}
		...
		%{\sf sm5+sl}	& 19.18 $\pm$ 0.32	& \vsx{0.97-1.02x   } & \vsx{1.28x} \\
...
amounts to the STM concurrency model \cite{stm-pldi06}%
\footnote{Proof left to future work. We've proved enough here already.}
and may reduce the state space even smaller than the original.
...
and produced the same 384-interleaving state space on {\tt counter}(2,2)
(in which DPOR triggers conflict aborts on every transaction anyway).
%and {\tt map\_basic}(2,1)'s 1-hour size estimate from 16388977 to 16126960. % about the same after all...
...
our running example program is still incorrect on Total Store Order (TSO) architectures such as x86.
...
On Partial Store Order (PSO) architectures, even more barriers may be necessary.
...
our current HTM implementation cannot find this bug.
In fact, it erroneously verifies the corresponding test {\tt htm2}(3,1)
% lol this is both old (quicksand) AND wrong (testing (2,1) which can't expose the bug)
%in 7 CPU-minutes,
...
we would urge any reader interested in using those to add them in by hand first.
...
We thank Mario Dehesa-Azuara for generously providing the HTM data structures used in our evaluation,
% no
%and the anonymous SIGBOVIK reviewers for their helpful comments.
the anonymous SIGBOVIK program committee,
and anyone who actually read this meta-joke of a non-joke paper in entirety.
