...
\begin{enumerate}
	...
	\item By the way, should the evaluations of MC research papers quantify variance in their CPU-time performance?
\end{enumerate}
...
For verification tests, we configured Landslide %'s Iterative Deepening algorithm
to optimize for completion time by prioritizing the maximal state space job and cancelling all others
({\tt -M} command-line flag),
a feature newly added since \cite{quicksand} and hence evaluated for the first time here.


\subsection{Variance}

Because many of the verification tests are long-running,
and we are writing this too close to the deadline (who doesn't),
we regret being unable to present every performance measurement above
as an average of multiple samples with error bars.
...
likely reuslting from kernel resource contention as Landslide uses {\tt fork()} to save simulation state,
...
\begin{table}[h]
	\begin{center}
	\begin{tabular}{l|c|c}
		workload & cputime (s) & vs baseline \\
		\hline
		{\sf none	} & 14.95 $\pm$ 0.17	& 1	\\
		{\sf vid-L	} & 15.13 $\pm$ 0.10	& 1.01x	\\
		{\sf ff/c	} & 15.55 $\pm$ 0.16	& 1.04x	\\
		{\sf ff/c+vid-S	} & 17.09 $\pm$ 0.34	& 1.14x	\\
		{\sf ls		} & 19.11 $\pm$ 0.32	& 1.28x	\\
		{\sf ff/c+ls	} & 19.66 $\pm$ 0.50	& 1.31x	\\
	\end{tabular}
	\end{center}
	\caption{Performance variance on {\tt counter(2,2)} with other programs running on the test machine.
		...
		Average of 10 samples, $\pm$ is 1 stddev.
	}
	\label{tab:variants}
\end{table}

We also show
in Table~\ref{tab:variants2}
...
as well as the variance of Landslide in bug-finding mode (i.e., without {\tt -M}).
%(2128 interleavings total).
Surprisingly, the {\tt bugfind} distribution was bimodal,
with 6 samples in a distribution of 106.14$\pm$1.22 and 4 in 142$\pm$1.67,
...
\begin{table}[h]
	...
	\begin{tabular}{l|r|r}
		...
		\hline
		{\sf verif} ({\tt -M})		& 403 & 14.95 $\pm$ 0.17	\\
		{\sf reinstrument}		& 403 & 24.72 $\pm$ 0.10	\\
		{\sf bugfind} (no {\tt -M})	& 2128 & 120.53 $\pm$ 18.62	\\
	\end{tabular}
	\end{center}
...
variables in the experimental environment
at least enough to ensure consistent performance between runs.
...
Otherwise, they should be shown only for a token small test
to assure the reader the environment is consistent in the larger tests,
...
In this evaluation, all other performance numbers were collected with
with Firefox and Chrome as the only other machine load.
