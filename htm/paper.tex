...
\begin{lemma}[Equivalence of Aborts]
	...
	\begin{itemize}
		\llitem $\tiiat{\alpha}$ be an HTM begin operation,
		\llitem $\tiiat{\beta_1}\dots\tiiat{\beta_n}$ be the transaction body (with $\beta_n$ the HTM end call),
		\llitem $\tiiat{\phi_1}\dots\tiiat{\phi_m}$ be the failure path, and
		\llitem $\tiiat{\omega_1}\dots\tiiat{\omega_l}$ be the subsequent code executed unconditionally.
	\end{itemize}
...
the execution with all successful transactions replaced by an abstract global lock.
...
\begin{proof}
We prove one case for each variant definiton for data races \cite{quicksand}.

\begin{itemize}
	\llitem {\bf Limited Happens-Before.}
	\llitem {\bf Pure Happens-Before.}
\end{itemize}
