...
We claim that MCing the interleaving $\ti@1, \tj@1-3, \ti@4-7$
...
of all subsets of $\tj@1-3$
with all subsets of $\ti@2a-2c$,
...
\begin{theorem}[Equivalence of Aborts]
	\label{thm:equiv}
	Let:%
	...
	\begin{itemize}
		\item $\tii@\alpha$ be an HTM begin operation,
		\item $\tii@\beta_1\dots\tii@\beta_n$ be the transaction body (with $\beta_n$ the HTM end call),
		\item $\tii@\phi_1\dots\tii@\phi_m$ be the failure path, and
		\item $\tii@\omega_1\dots\tii@\omega_l$ be the subsequent code executed unconditionally.
	\end{itemize}
	Then, for any interleaving prefix
	\[
		\tii@\alpha,\tii@\beta_1\dots\tii@\beta_b,\tjj@\gamma_1\dots\tjj@\gamma_j,\tkk@\kappa_1\dots\tkk@\kappa_k
	\]
	with $b<n$, $j \ne i$, $k \ne i$, etc., either:
	\begin{enumerate}
		\item $\tii@\alpha,\tii@\beta_1\dots\tii@\beta_b\dots\tii@\beta_n,\tjj@\gamma_1\dots\tjj@\gamma_j,$ \\
			$\tkk@\kappa_1\dots\tkk@\kappa_k$
			(independent case), or
		\item $\tii@\alpha,\tjj@\gamma_1\dots\tjj@\gamma_j,\tkk@\kappa_1\dots\tkk@\kappa_k,\tii@\phi_1\dots$
			(conflicting case)
	\end{enumerate}
...
From Theorem~\ref{thm:equiv} we have that the body of all transactions may be executed atomically within the MC environment.
