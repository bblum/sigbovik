...
which we will use in our evaluation following.
...
\begin{table*}[t]
	...
	\begin{tabular}{cc||r|r|r||r|r|r|r}
		...
		\cline{2-9}
			...
			& 4,2	&	&	&	& 674.87& 650.58& 16511	& 13755188 \\
	\end{tabular}
	\end{center}
	\caption{Landslide's bug-finding performance on various test configurations.
		...
		plus the number of interleavings of the ultimately buggy state space.
		Landslide's state space size estimation \cite{estimation},
...
\footnote{This was necessary for {\tt swapbug(4,1)} to trigger a switch to the buggy state space.}
shown left,
as well as to prioritize the maximal state space as discussed above,
shown right.
We make the following observations/conclusions:
\begin{enumerate}
	\item The multiplicative factor in bug-finding speed (2-4x, eyeballing) is generally smaller
		than that of the total number of interleavings (10-30x)
...
\begin{table}[t]
	...
	\begin{tabular}{l|c|c|c}
		workload & cputime (s) & vs self avg & vs baseline \\
		\hline
...
Otherwise, considering the maximum variance of 5\% in Table~\ref{tab:variants},
%(2\% in reasonable loads),
they should be shown only for a token small test
to assure the reader the environment is consistent for the larger tests
...
Outside of Table~\ref{tab:variants}, we collected all performance numbers
