...
pub trait BisectStrategy {
    fn select_commit(&mut self, state: &BisectState) -> usize;
}

struct SimulateResult {
    steps: usize,
    confidence: f64,
    suspected_buggy_commit: usize,
}

impl BisectState {
    fn new(num_commits: usize, false_positive_rate: f64) -> Self {
    ...
    fn simulate_while(mut self, mut strategy: impl BisectStrategy, f: impl Fn(&Self) -> bool) -> SimulateResult {
        while f(&self) {
            self.simulate_step(strategy.select_commit(&self));
        }
    ...
    fn simulate_til_max_steps(self, strategy: impl BisectStrategy, max_steps: usize) -> SimulateResult {
        self.simulate_while(strategy, |state| state.history.len() < max_steps)
    }

    fn simulate_til_confident(self, strategy: impl BisectStrategy, confidence_threshold: f64) -> SimulateResult {
        self.simulate_while(strategy, |state| state.best_commit().1 < confidence_threshold)
    }
}

fn main() {
    BisectState::new(65536, 0.0).assert_consistent();
}
